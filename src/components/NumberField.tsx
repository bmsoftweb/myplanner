...
interface NumberFieldProps {
  ...
  /** Tira o ponto de milhar: para número que não é quantidade, como um ano */
...
export const NumberField: React.FC<NumberFieldProps> = ({
  ...
  return (
    <input
      ...
      autoComplete="off"
      value={paraExibicao(atual, scale, semAgrupamento)}
      onChange={(e) => {
        moverCursorParaFim.current = true;
        onChange(paraCanonico(aplicarDigitacao(e.target.value, atual, scale, allowNegative, semAgrupamento), scale));
      }}
      onPaste={(e) => {
        e.preventDefault();
      ...
      required={required}
      disabled={disabled}
      placeholder={placeholder ?? paraExibicao({ negativo: false, digitos: '0' }, scale, semAgrupamento)}
