 ...
 *
 * `semAgrupamento` tira o ponto de milhar. É o que se usa em número que não é
 * quantidade — um ano, por exemplo, que deve aparecer como 2026 e não 2.026.
 */
export function paraExibicao(
  { negativo, digitos }: EstadoNumero,
  scale: number,
  semAgrupamento = false,
): string {
  if (digitos === '') return '';
  const cheio = digitos.padStart(scale + 1, '0');
  const bruto = cheio.slice(0, cheio.length - scale) || '0';
  const inteiro = semAgrupamento ? bruto : bruto.replace(/\B(?=(\d{3})+(?!\d))/g, '.');
...
export function aplicarDigitacao(
  ...
  const exibicaoAtual = paraExibicao(atual, scale, semAgrupamento);
